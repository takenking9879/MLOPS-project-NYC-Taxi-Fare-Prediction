...
from src.CT.data.data_download_ct import DataDownloadCT
import requests
import os

logger = create_logger('data_download_cm', 'logs/data_download_cm.log')

class DataDownloadCM(DataDownloadCT):
    def __init__(self, params_path: str, new_data_original_dir: str):
        super().__init__(params_path=params_path, new_data_original_dir=new_data_original_dir )
        self.params = self.load_params()["continuous_monitoring"]
        self.retrain = True
        self.params = self.params["data_download_cm"]
    ...
    def descargar_parquets(self, start_year: int, start_month: int) -> None:
        ...
        for i in range(1):  # mes inicial + 2 siguientes
            year = start_year
            month = start_month + i
...
def main():
    try:
        ...
        new_data_original_dir = os.path.join(root_dir, 'CM_data/original')

        download = DataDownloadCM(params_path, new_data_original_dir)
