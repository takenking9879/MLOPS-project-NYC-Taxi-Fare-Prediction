from src.CT.data.data_osrm_ct import DataOSRMCT
...
class DataOSRMCM(DataOSRMCT):
    def __init__(self, params_path: str, preprocessed_data_dir: str = None, preprocessed_osrm_data_dir: str = None, artifacts_dir: str = None):
        super().__init__(params_path=params_path, preprocessed_data_dir=preprocessed_data_dir, preprocessed_osrm_data_dir=preprocessed_osrm_data_dir, artifacts_dir= artifacts_dir)
        self.preprocessed_osrm_data_dir = preprocessed_osrm_data_dir or os.path.join("CM_data", "processed_osrm")
        self.preprocessed_data_dir = preprocessed_data_dir or os.path.join("CM_data", "processed")
        all_params = self.load_params()["continuous_monitoring"]
        self.retrain = True
        self.params = all_params["data_osrm_cm"]
        ...
        try:
            ...
            for f in all_files:
                ...
                if f.endswith('.parquet'):
                    ...
                    df = self.osrm_feature(df, use_csv_path=os.path.join("CM_data/osrm", "osrm_routes.csv"),
...
def main():
    try:
        ...
        processed_osrm_data_dir = os.path.join(root_dir, 'CM_data/processed_osrm')
        processed_data_dir = os.path.join(root_dir, 'CM_data/processed')
        artifacts_dir = os.path.join(root_dir, 'artifacts')

        preprocessing = DataOSRMCM(params_path,processed_data_dir,processed_osrm_data_dir, artifacts_dir)
        preprocessing.add_features()

        remove_temp_csv = '../../../CM_data/osrm/osrm_routes.csv'
